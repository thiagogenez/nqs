...
for i in $(seq 1 $AMOUNT_OF_SIMULATION); do
	...
	# For each service rate in SERVICE_RATES
		...
		echo "Simulation #$i with service rate $rate Mbit/s";
			...
			python $SIMULATOR_PATH -o $OUTPUT_PATH/$i-serviceRate-$rate-Mbps-$ARRIVAL_RATE-Mbps.out -s $rate$UNIT -a $ARRIVAL_RATE$UNIT

		#else		
		#	python $SIMULATOR_PATH -o $OUTPUT_PATH/$i-serviceRate-$rate-Mbps-$ARRIVAL_RATE-Mbps.out -s $rate$UNIT -i $OUTPUT_PATH/traffic.trace
		#fi	
	done
