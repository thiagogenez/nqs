...
for rate in ${SERVICE_RATES[@]}; do
	...
	i=0
	for data in ${DATA[@]}; do
		array[i]=`cat $OUTPUT_PATH/*serviceRate-$rate-bps* | grep "$data" | awk -F " " {'print $5'} | awk '{sum+=$1; sumsq+=$1*$1;} END {avg=sum/NR; std=sqrt(sumsq/NR - avg^2); cil95=(1.96 * std)/sqrt(NR); printf "%.12f %.12f %.12f %.12f %.12f\n", avg, std, ci95, avg-ci95, avg+ci95}' | awk '{print $1, $3}'` 
		((i++))
	done
